...
class UPISimulator:
    ...
    def inject_adversarial_burner_swarm(self):
        """Mules controlled from a single device, bypassing other rules via low volume."""
        total_amount = random.uniform(5000, 15000) # Small amount to avoid Z-score
        num_mules = random.randint(4, 7) # 4 to 7 mules on one device
        chain_id = f"CHAIN_ADV_BURNER_{uuid.uuid4().hex[:6].upper()}"
        ext_source = f"EXT_VICTIM_{uuid.uuid4().hex[:4].upper()}"
        start_time = self.random_timestamp() - timedelta(days=2)
        shared_dev = f"DEV_BURNER_{uuid.uuid4().hex[:8].upper()}"
        
        # Slow moving, small amounts, 1 hop -> bypasses velocity, zscore, and fanout (each is 1-to-1)
        cashout = self.generate_account(is_mule=True, behavior="mule_cashout", is_adversarial=True)
        
        for i in range(num_mules):
            node = self.generate_account(is_mule=True, behavior="mule_hop", shared_device=shared_dev, is_adversarial=True)
            jitter_amt = (total_amount / num_mules) * random.uniform(0.9, 1.1)
            in_time = start_time + timedelta(hours=i*2) # Space them out
            out_time = in_time + timedelta(hours=random.randint(26, 48)) # >24h to bypass velocity
            
            self.add_transaction(ext_source, node, jitter_amt, in_time, chain_id, True)
            self.add_transaction(node, cashout, jitter_amt, out_time, chain_id, True)
    ...
    def generate_full_dataset(self, num_accounts=6000, mule_prevalence=0.03):
        ...
        for _ in range(15): self.inject_adversarial_burner_swarm() # Round 4 (Device Linked)
